...
def train(
    ...
    for iB, t in enumerate(train_loader):
        ...
        pr_wv_str, pr_wv_str_wp = convert_pr_wvi_to_string(
            pr_wvi, nlu_t, nlu_tt, tt_to_t_idx
        )
...
def test(
    ...
    for iB, t in enumerate(data_loader):
        ...
        if not EG:
            ...
            # Following variables are just for the consistency with no-EG case.
        ...
        for b, pr_sql_i1 in enumerate(pr_sql_i):
            results1 = {"query": pr_sql_i1, "table_id": tb[b]["id"], "nlu": nlu[b]}
            results.append(results1)
...
def tokenize_stanza(nlu1):
    nlu1_tok = []
    nlp = stanza.Pipeline(lang="en", processors="tokenize")
    doc = nlp(nlu1)
    for i, sentence in enumerate(doc.sentences):
        for tok in sentence.tokens:
            nlu1_tok.append(tok.text)
    return nlu1_tok
...
def find_table_by_id(data_table, table_name):
...
def infer(
    ...
    # I know it is of against the DRY principle but to minimize the risk of introducing bug w, the infer function introduced.
    model.eval()
    model_bert.eval()
    ...
    # Get inputs
    nlu = [nlu1]
    nlu_t1 = tokenize_stanza(nlu1)
    nlu_t = [nlu_t1]

    tb1 = find_table_by_id(data_table, table_name)
    # tb1 = data_table[0]
    print(tb1["id"])
    hds1 = tb1["header"]
    tb = [tb1]
    ...
    try:
        pr_ans, _ = engine.execute_return_query(
            tb[0]["id"], pr_sc[0], pr_sa[0], pr_sql_i[0]["conds"]
        )
    except:
...
if __name__ == "__main__":
    ...
    path_wikisql = os.path.join(".", "data")
    ...
    path_db = "./data"
    db_name = "test"
    data_table = load_jsonl("./data/test.tables.jsonl")
    n_Q = 5
    for i in range(n_Q):
        topic = input("Type topic: ")
        question = input("Type question: ")
        table_name = find_table_by_topic(topic, question, data_table)
