...
class Seq2SQL_v1(nn.Module):
    def __init__(self, iS, hS, lS, dr, n_cond_ops, n_agg_ops, old=False):
        ...
        self.wvp = WVP_se(
            iS, hS, lS, dr, n_cond_ops, old=old
    ...
    def forward(
        ...
        # wc
        s_wc = self.wcp(
            wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_wc=show_p_wc, penalty=True
        )
        ...
        # wo
        s_wo = self.wop(
            wemb_n, l_n, wemb_hpu, l_hpu, l_hs, wn=pr_wn, wc=pr_wc, show_p_wo=show_p_wo
        )
    ...
    def beam_forward(
        ...
        for i_beam in range(beam_size):
            ...
            s_sa = self.sap(
                wemb_n, l_n, wemb_hpu, l_hpu, l_hs, pr_sc, show_p_sa=show_p_sa
            )
        ...
        for i_op in range(self.n_cond_ops - 1):
            ...
            F.softmax(s_wv, dim=-2).detach().to("cpu").numpy()
        ...
        for b, idxs1 in enumerate(idxs):
            ...
            for i_wn, idxs11 in enumerate(idxs1):
                ...
                temp_pr_wv_str, _ = convert_pr_wvi_to_string(
                    [[wvi]], [nlu_t[b]], [nlu_wp_t[b]], [wp_to_wh_index[b]]
                )
...
class SCP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_sc=False):
        # Encode
        wenc_n = encode(
            self.enc_n, wemb_n, l_n, return_hidden=False, hc0=None, last_only=False
        )  # [b, n, dim]
        ...
        if show_p_sc:
            ...
            for i_h in range(l_hs[0]):
                ...
                plot(
                    p_n[0][i_h][:].data.numpy() - i_h,
                    "--" + _symbol[color_idx] + _color[color_idx],
                    ms=7,
                )
...
class SAP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3, n_agg_ops=-1, old=False):
        ...
        self.W_att = nn.Linear(hS, hS)
        self.sa_out = nn.Sequential(
            nn.Linear(hS, hS), nn.Tanh(), nn.Linear(hS, n_agg_ops)
        )  # Fixed number of aggregation operator.
...
class WNP(nn.Module):
    def __init__(
        ...
        self.wn_out = nn.Sequential(
            nn.Linear(hS, hS), nn.Tanh(), nn.Linear(hS, self.mL_w + 1)
        )  # max number (4 + 1)
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_wn=False):
        ...
        hidden = hidden.view(
            bS, self.lS * 2, int(self.hS / 2)
        )  # [4, B, 100/2] # number_of_layer_layer * (bi-direction) # lstm input convention.
        ...
        att_n = self.W_att_n(wenc_n).squeeze(
            2
...
class WOP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3, n_cond_ops=3):
        ...
        self.wo_out = nn.Sequential(
            nn.Linear(2 * hS, hS), nn.Tanh(), nn.Linear(hS, n_cond_ops)
        )
    ...
    def forward(
        ...
        att = torch.matmul(
            self.W_att(wenc_n).unsqueeze(1), wenc_hs_ob.unsqueeze(3)
        ).squeeze(3)
...
class WVP_se(nn.Module):
    ...
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3, n_cond_ops=4, old=False):
        ...
        if old:
            ...
            self.wv_out = nn.Sequential(
                nn.Linear(4 * hS, hS), nn.Tanh(), nn.Linear(hS, 2)
            )
        # self.wv_out = nn.Sequential(
        #     nn.Linear(3 * hS, hS),
        #     nn.Tanh(),
    ...
    def forward(
        ...
        # Encode
        if not wenc_n:
            wenc_n, hout, cout = encode(
                self.enc_n, wemb_n, l_n, return_hidden=True, hc0=None, last_only=False
            )  # [b, n, dim]
        ...
        vec = torch.cat(
            [self.W_c(c_n), self.W_hs(wenc_hs_ob), self.W_op(wenc_op)], dim=2
        )

        # Make extended vector based on encoded nl token containing column and operator information.
        # wenc_n = [bS, mL, 100]
        # vec2 = [bS, 4, mL, 400]
        vec1e = vec.unsqueeze(2).expand(
            -1, -1, mL_n, -1
        )  # [bS, 4, 1, 300]  -> [bS, 4, mL, 300]
        wenc_ne = wenc_n.unsqueeze(1).expand(
            -1, 4, -1, -1
